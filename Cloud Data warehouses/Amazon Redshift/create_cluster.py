...
def create_iam_role(iam, DWH_IAM_ROLE_NAME):
    ...
    # creates IAM role and attaches necessry policies to use AWS redshift services.
...
def create_redshift_cluster(redshift, roleArn, DWH_CLUSTER_TYPE, DWH_NODE_TYPE, DWH_NUM_NODES, DWH_DB, DWH_CLUSTER_IDENTIFIER, DWH_DB_USER, DWH_DB_PASSWORD):
    ...
    # Using the arguments creates Amazon redshift cluster.
...
def get_cluster_props(redshift, DWH_CLUSTER_IDENTIFIER):
    ...
    # returns redishift cluster properties
...
def prettyRedshiftProps(props):
    # Returns the dataframe of cluster attributes
...
def open_ports(ec2, myClusterProps, DWH_PORT):
    # creates necessary access to cluster with port mapping
...
def main():
    ...
    time.sleep(300)
    # wait for cluster to get set up
